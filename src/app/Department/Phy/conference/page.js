...
const PhyConferencePage = () => {
  ...
  const fetchPublications = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/conference?type=phy&page=${page}&limit=${limit}`
      );
                  ...
                  >
                    ...
                    {paper.doi && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-block mt-3 text-sm font-medium text-white bg-blue-600 px-4 py-1.5 rounded hover:bg-blue-700 transition"
                      >
                        View DOI
                      </a>
                    )}
