...
const Page = () => {
  ...
  useEffect(() => {
    const fetchAcademics = async () => {
      setIsLoading(true);
      try {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/notice?type=academics&page=${page}&limit=${limit}`;
        ...
        setAcademics(
          notices.filter((notice) => notice.isVisible === 1)
        );
  ...
  // ✅ Safe link parser
  const getParsedLink = (notice_link) => {
    try {
      return notice_link ? JSON.parse(notice_link).url : null;
    } catch {
      return null;
    }
  };
  ...
  return (
    <div className="p-5 md:p-10 md:pl-28 md:pr-28">
      <h2 className="text-2xl text-center pb-7 md:pb-10 text-red-950 font-bold">
        Academic Notices
      </h2>
        ...
        <>
            ...
            {academics.length === 0 ? (
              ...
              academics.map((notice) => (
                <Noticecard
                  ...
                  attachments={notice.attachments}
                  imp={notice.important}
                  link={getParsedLink(notice.notice_link)}
                />
              ))
            )}
          </div>
          ...
          <div className="flex justify-center items-center gap-4 mt-6">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 bg-red-500 text-black rounded disabled:opacity-50"
            >
              Previous
            </button>
            ...
            <button
              ...
              className="px-4 py-2 bg-red-400 text-red-900 rounded disabled:opacity-50"
            >
              Next
            </button>
          </div>
